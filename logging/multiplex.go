...
func (m *connTracerMultiplexer) ReceivedVersionNegotiationPacket(hdr *Header) {
	for _, t := range m.tracers {
		t.ReceivedVersionNegotiationPacket(hdr)
	}
}
...
func (m *connTracerMultiplexer) ReceivedStatelessReset(token *[16]byte) {
	for _, t := range m.tracers {
		t.ReceivedStatelessReset(token)
	}
}
